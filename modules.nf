process PRIPRAVA {
        tag "downsampling 1 on $name using $task.cpus CPUs and $task.memory memory"
        input:
        ...
        vara=\$(zcat ${name}_R1.fastq.gz | wc -l)
        if [ \$vara  -gt 4000000 ]; then
        echo downsampling
        seqtk sample -s100 ${name}_R1.fastq.gz 1000000 > 1mil-reads${name}_R1.fastq
        gzip 1mil-reads${name}_R1.fastq
        seqtk sample -s100 ${name}_R2.fastq.gz 1000000 > 1mil-reads${name}_R2.fastq
        gzip 1mil-reads${name}_R2.fastq

        else
        echo vse ok
        cp ${name}_R1.fastq.gz podmil-reads${name}_R1.fastq.gz
        cp ${name}_R2.fastq.gz podmil-reads${name}_R2.fastq.gz
        fi
        """
}
